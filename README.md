...
| **Resposta recebida** | A IA estruturou listas separadas para RFs, RNFs e RNs, sugerindo detalhes como tempo real para baixa de estoque, restrições de LGPD no prontuário médico e validação de saldos no caixa antes da finalização da venda. |
...
| **Trechos rejeitados ou corrigidos** | Rejeitado: A sugestão inicial da IA de criar um requisito complexo de controle de comissão para atendentes. Como isso não era foco do escopo atual da clínica, o grupo optou por focar apenas na auditabilidade (RN05). <br><br> Corrigido: Ajuste no RF05 e RNF01 para especificar que a baixa de estoque e a validação ocorrem em tempo real e de forma integrada ao item de venda, refletindo exatamente a lógica do nosso DER. |
| **Justificativa da escolha final** | Os requisitos e regras finais foram selecionados e adaptados para refletirem fielmente as prioridades do cliente (petshop), garantindo que o sistema atenda tanto a parte fiscal e de estoque quanto o atendimento clínico diário. |
| **Reflexão crítica** | A IA ajuda muito a organizar e padronizar o formato dos requisitos de software, garantindo clareza técnica. No entanto, a definição exata de quais restrições são vitais para o negócio — como a inviolabilidade do histórico clínico ou a capacidade operacional de banho e tosa — exigiu a nossa percepção crítica da realidade da empresa. |
